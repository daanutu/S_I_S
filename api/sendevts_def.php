...
if(isset($_GET['parms'])) { //parms esiste
	...
	if(!is_null($parms)){ //stringa JSON valida
		...
		if($err==0){ 	//la string JSON contiene tutti i dati
			...
			$mysqli = new mysqli('localhost','root','','scuole_in_sicurezza');
			if($mysqli->connect_errno==0) {
				...
				foreach ($events as $event){ 	//per ogni elemento di events
					...
					if($evt=="1") { 	//entrata in allarme
						//prepara query per aggiungere relazione in allarmi_in_corso
						$q="INSERT INTO allarmi_in_corso VALUES ('$mymac', '$adr')";
						...
						//toglie incondizionatamente la relazione in allarmi_in_corso
						$q="DELETE FROM allarmi_in_corso WHERE btmac1='$mymac' AND btmac2='$adr'";
						$result = $mysqli -> query($q);
						if($result==false) {
							$error=$mysqli->errno;		
						}
					...
					//determina se mymac è in allarme (almeno un elemento di allarmi_in_corso contiene mymac)
					$q="SELECT * FROM allarmi_in_corso WHERE btmac1='$mymac' OR btmac2='$mymac'";
					$result = $mysqli -> query($q);
					if($result==false) {
						$error=$mysqli->errno;		
					}				
					...
					//verifica se mymac è in allarme (almeno una riga in allarmi_in_corso)
					...
					//determina se othermac è in allarme (almeno un elemento di allarmi_in_corso contiene other)
					$q="SELECT * FROM allarmi_in_corso WHERE btmac1='$adr' OR btmac2='$adr'";
					$result = $mysqli -> query($q);
					if($result==false) {
						$error=$mysqli->errno;		
					}					
